...
async def cmd_help(message: Message) -> None:
    ...
    mssg_txt1 += 'Имена участников не должны содержать символа ";" или являться словом "Отправить".\n'
    ...
    mssg_txt1 += "Также данный бот при создании отчётов будет сохранять дату их создания для подведения статистики или создания файла отчётов за определённый промежуток времени.\n"
    ...
    mssg_txt1 += "По умолчанию бот будет работать в часовом поясе UTC +3:00.\n"
    mssg_txt1 += "Если вы живёте в другом часовом поясе, то рекомендуется указать его боту при помощи специальной команды.\n"
    mssg_txt1 += 'Время указывается в формате "Ч:ММ" со знаком "+", если смещение положительное, иначе со знаком "-".\n\n'
    ...
    mssg_txt2 += "/cancel - Прервать выполнение текущей команды;\n"
    ...
    mssg_txt2 += (
        "/getstatistics - Получить статистику об отсутствии участников группы.\n"
    )
    mssg_txt2 += (
        "/getreportsfile - Получить файл отчётов об отсутствии участников групп"
    )
